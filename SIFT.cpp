...
{
	printf("SIFT:Generating scale space...\n");
	// For loops
	unsigned int i,j;
...
void SIFT::DetectExtrema()
{
	...
	printf("SIFT:Rejected %d keypoints\n", numRemoved);
}
...
{
	printf("SIFT:Extract keypoint descriptors...\n");
	...
	for(unsigned int ikp = 0;ikp<m_numKeypoints;ikp++)
	{
		...
		for(unsigned int orient_count=1;orient_count<mag.size();orient_count++)
		{
			if(mag[orient_count]>main_mag)
			{
				...
				//if(ikp==1)printf("\#\tOrientation\tMagnitude\n");
				//printf("%d\t%f\t%f\n",ikp,main_orien,main_mag);
			}
		}
	...
	//printf("X2:%d,Y2:%d,fv2:%f\n",m_keyDescs[m_numKeypoints-100].xi,m_keyDescs[m_numKeypoints-100].yi,m_keyDescs[m_numKeypoints-100].fv[0]);
...
#ifdef FLINN
//make cvMat from keyDesc and store it
CvMat* SIFT::make_desc_mat(){
	static int a=0;
	float factor=0.3;

	CvMat* keydesc_mat = cvCreateMat(m_numKeypoints,m_keyDescs[0].fv.size(), CV_32FC1);
	//printf("i=%d,j=%d",m_numKeypoints,m_keyDescs[0].fv.size());

	for(double i=0;i<m_numKeypoints;i++)
	{
		for(double j=0;j<m_keyDescs[0].fv.size();j++)
		{
			cvSetReal2D(keydesc_mat,i,j,(double)m_keyDescs[m_numKeypoints-1].fv[0]);
		}
	}
	
	CvMat* keypoint_desc = cvCloneMat(keydesc_mat);

	char* filename = new char[100] ;
	sprintf(filename, "KeyDesc-%d.jpg",a);
	cvSaveImage(filename, keydesc_mat);
	//printf("image_saved:%d\n",a);

// Optional exaustive search

	//old_numKeypoint = 3;
	//int feature_vector_matched_count=0;
	/*
	if(a==1)
	{
		printf("matching. . ");
		printf("m=%d,k=%d,w=%d",m_numKeypoints,m_keyDescs[0].fv.size(),old_numKeypoint);
		for(double m=0;m<old_numKeypoint;m++)
		{
			for(double k=0;k<m_numKeypoints;k++)
			{
				for(double w=0;w<m_keyDescs[0].fv.size();w++)
				{
					double object_fv,image_fv;
					object_fv = cvGetReal2D(object_desc,(int)m,(int)w);
					image_fv =	cvGetReal2D(image_desc,(int)k,(int)w);
					printf("%f %f\n",object_fv,image_fv);
					if(object_fv >= (image_fv*factor) && object_fv <= (image_fv*factor))
					{
						printf("feature_vector#: %d of key_poin #:%d matched !!",w,m);
						feature_vector_matched_count++;
						if(feature_vector_matched_count>20) printf("Object Keypoint;%d matched !!",m);
...
void SIFT::ShowKeypoints()
{
	...
	char* windowname = new char[200];
	sprintf(windowname, "KeyImage-%d.",a);
